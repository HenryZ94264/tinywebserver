...
void WebServer::init(int port, string user, string passWord, string databaseName, int log_write, 
                     int opt_linger, int trigmode, int sql_num, int thread_num, int close_log, int actor_model, int timer_type)
{
    m_port = port;
    m_user = user;
    m_passWord = passWord;
...
void WebServer::sql_pool()
{
    ...
    // 初始化数据库读取表
    users->initmysql_result(m_connPool);    // 将数据库中的数据载入到服务器中
}

void WebServer::thread_pool()
{
    //线程池
    m_pool = new threadpool<http_conn>(m_actormodel, m_connPool, m_thread_num);     // 建立http连接线程池
}
...
void WebServer::eventListen()
{
    ...
    assert(ret != -1);  
    utils.setnonblocking(m_pipefd[1]);  // 管道写端为非阻塞
    utils.addfd(m_epollfd, m_pipefd[0], false, 0);  // 将管道读端注册到epoll中，设置为ET非阻塞

    utils.addsig(SIGPIPE, SIG_IGN);
    // 传递给主循环的信号值
    utils.addsig(SIGALRM, utils.sig_handler, false);
    utils.addsig(SIGTERM, utils.sig_handler, false);
    // 系统调用 每隔TIMESLOT时间触发SIGALRM信号
    alarm(TIMESLOT);
...
void WebServer::adjust_timer(base_timer *timer)
{
    if(m_timer_type == 0)
    {
        ...
        utils.m_timer->adjust_timer(timer);  // 更新定时器超时值
        LOG_INFO("%s", "adjust timer once");
    }
    else if(m_timer_type == 1)  // 时间轮的调整
    {
...
void WebServer::deal_timer(base_timer *timer, int sockfd)
{
    if(m_timer_type == 0)
    {
...
{
    base_timer *timer = users_timer[sockfd].timer;
    ...
    {
        //proactor
        if (users[sockfd].read_once())  // read_once 仅进行一次读取
        {
